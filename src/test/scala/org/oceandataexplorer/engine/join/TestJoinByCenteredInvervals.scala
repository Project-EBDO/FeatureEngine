...
/**
 * JoinByCenteredInvervals test implementation
 *
 * @author Alexandre Degurse
 */
class TestJoinByCenteredInvervals extends FlatSpec with SharedSparkContext with Matchers {

  val toValue = udf[Double, Timestamp]((ts: Timestamp) => ts.toInstant.toEpochMilli.toDouble)

  it should "join two timeseries with the same timestamps" in {
    ...
    val jn = JoinByCenteredInvervals(spark)
    val dfMerged = jn(df1, df2, 1000L)
  ...
  it should "raise an IllegalArgumentException when given a negative time granularity" in {
    ...
    an[IllegalArgumentException] should be thrownBy jn(df, df, -1L)
    an[IllegalArgumentException] should be thrownBy jn(df, df, 0)
  }
}
